...
Resolve piece files relative to the kit document. Every plan receives the same complete catalogue and GLB bytes. Apply configuration selection, node transforms, local offset, optional closure scale, rotation, translation and receiving clips as defined by the [contract](CONTRACT.md). Only fractional 2 m closures scale along their run. Markings, scan quads, palette regions, wear and marquee text are placement attributes. `placementFootprint` returns the clipped world footprint. Register MeshoptDecoder and bind the exact native material snapshot. `report.profiles` lists widths mapped to the nearest catalogue profile.
