...
export function createHardware(options: FurnitureOptions): FurnitureModel {
  if (!options || !['guard', 'inlet', 'channel', 'access', 'cable', 'marquee', 'tree-grate'].includes(options.kind)
    ...
    || ['cable', 'marquee', 'tree-grate'].includes(options.kind) && (options.length < 1 || options.depth < 0.4)) throw invalidParams('Invalid native hardware options');
  ...
  else if (options.kind === 'marquee') marquee(builder, options);
