...
`ProfileCatalogue` reports nearest width mappings. `UnitPlan` partitions runs and junctions. `StationPlacements` fits whole pedestrian pieces around shafts. Fractional remainders scale plain closures. `StreetUnits` emits transforms and shader values under the [placement contract](../../../CONTRACT.md), with feature placement indices retained separately in the manifest.
