...
export function constructPiece(scene: SceneInput, seed: number): ConstructedPiece {
  ...
  for (const owner of a.owners) {
    ...
    const features = details.get(owner.id) ?? [], planned = a.format === 'district' ? ramps.plan(owner) : [];
    ...
    output.meshes = output.meshes.map(mesh => unitBounds(mesh, a.bounds)).filter(mesh => mesh.positions.length);
    outputs.push(output);
  }
