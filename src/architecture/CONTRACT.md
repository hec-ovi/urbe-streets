...
Median stations and widths match their straight road, footprint and paving match the physical owner's ground, and ornament anchors stay inside paving. Opposite avenue lane pairs leave the declared median clear. Parking slots stay inside their owned bay; walking clearance equals paved width minus parking depth. A bay that breaks those rules is dropped and returned in `degraded` with its id and reason, so one bay never stops a plan. Malformed format, style, dimensions or geometry fail with `E_UNSUPPORTED_ARCHITECTURE` and a field path.
