...
export async function buildNative(request:NativeStreetRequest,options:NativeBuildOptions):Promise<NativeStreetBuild>{
  let output:Output|undefined,features:FeatureBuilder|undefined;
  try{
    ...
    const wear=(point:Parameters<WearField['sample']>[0])=>field.sample(point),partition=new NativePartition(),coverage=new NativeCoverage(a),edges=new EdgeRing(a),paving=new Paving(a,request.seed,wear),markings=new Markings(a,request.seed,wear);
    features=new FeatureBuilder(a,request.seed,wear);const plan=features.plan(),shafts=a.shafts.map(shaft=>shaft.ring);let panels=0;
    ...
    for(const owner of a.owners){
      ...
      asphalt(owner,batch,shafts);panels+=parking(owner,batch);edges.build(owner,batch,cuts,shafts);panels+=paving.build(owner,batch,cuts,shafts);markings.build(owner,batch);
      const surface=batch.finish(),hardware=features.draw(owner,selected);coverage.add(owner,[...surface.coverage,...hardware.coverage]);
    ...
    const manifest:NativeStreetManifest={
      ...
      pieces:[],ground:coverage.finish(),features:plan.map(feature=>feature.descriptor),materials:{mode:'native-reference',binding:catalog.binding},wear:field.snapshot(),protected:a.protections,
      ...
      statistics:{pieces:0,triangles:0,materials:surfaces.size,groundOwners:a.owners.reduce((sum,owner)=>sum+owner.ground.length,0),features:plan.length,panels},
    };
